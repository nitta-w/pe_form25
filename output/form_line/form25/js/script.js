...
$(() => {
  ...
  function onWindowLoad() {
    ...
    setTimeout(() => {
      [
        ...
        'pose_4_naki.webp',
        'pose_5_kangae.webp',
        'pose_6_hirameki.webp',
        'pose_8_guts.webp',
        'fat_1_s.webp',
        'fat_3_l.webp',
        'fat_4_niyari.webp',
        'fat_5_warai.webp',
        'fat_6_dakitsuki.webp',
        'fat_8_haretsu.webp',
        'fat_9_shometsu.webp',
        'prop_1_sofa.webp',
        'prop_2_parfait.webp',
  ...
  const addParamsToCtaUrl = () => {
    ...
    const bodyPart = `,${$('[name="body_parts"]')
      .map((_, el) => $(el).val())
      .get()
      .join(',')},`;
    const dietHistory = `,${$('[name="diet_history"]')
      .map((_, el) => $(el).val())
      .get()
      .join(',')},`;
    ...
    const isClinicListError = $('[name="clinic_shop"]').is(
      '[data-clinic-list-error]',
    );
    // カレンダーの表示エラーのフラグ
    const isCalendarError = $('[data-tc-is-error]').length > 0;

    // 希望日は1日のみ選択（date_1）、その日の希望時間を第1〜第3希望として3つ選択する
    const requestDate = $('[name="date_1"]:checked').val() || '';
    ...
    // 選択日がクーポン対象日かどうか（calendar-spec.md §3-2-1）
    const isCouponDay =
      $('[name="date_1"]:checked').attr('data-tc-is-coupon-day') === 'true';
    // 選択日が当日（スケジュール初日）かどうか
    const isToday =
      $('[name="date_1"]:checked').attr('data-tc-is-today') === 'true';
    ...
    switch (botBasicId) {
      case '897vblrf':
        ...
        if (!isClinicListError && !isCalendarError) {
          ...
          varMapping['2460256'] = isCouponDay ? '日時特典あり' : '日時特典なし';
        ...
        varMapping['2184987'] = gender; // 性別
        varMapping['2184988'] = age; // 年齢
        varMapping['2184994'] = bodyPart; // 部位
        varMapping['2613978'] = dietHistory; // 試したことがあるダイエット
        varMapping['2504096'] = errorCodes; // エラーコード
  ...
  const SND = (() => {
    ...
    return {
      ...
      pop() {
        tone(392, 0.16, 'sine', 0.14);
        tone(523, 0.12, 'sine', 0.1, 0.05);
      },
      burst() {
        tone(784, 0.1, 'square', 0.07);
        tone(1047, 0.25, 'sine', 0.1, 0.06);
        tone(1568, 0.3, 'sine', 0.06, 0.12);
      },
      levelup() {
        [523, 659, 784, 1047].forEach((f, i) =>
          tone(f, 0.18, 'triangle', 0.12, i * 0.09),
        );
      },
      gaan() {
        tone(196, 0.55, 'sawtooth', 0.07);
        tone(185, 0.55, 'sawtooth', 0.05, 0.03);
      },
      blip() {
        tone(1250, 0.025, 'square', 0.02);
      },
      walkOn() {
        mood = 'walk';
      },
  ...
  const GIRL = (() => {
    ...
    function pose(file) {
      ...
      if (NEGA_POSES.some((n) => file.includes(n))) SND.gaan();
      else SND.levelup();
    }
    function unpose() {
  ...
  const FAT = (() => {
    ...
    function update(n) {
      counter.textContent = '脂肪くん×' + n;
      counter.classList.toggle('is-show', !!n);
    }
    return {
      add(size) {
        ...
        el.style.animationDelay = -list.length * 0.7 + 's';
        el.style.animationDuration = 2.2 + (list.length % 3) * 0.5 + 's';
        const file =
          size === 'L' ? 'fat_3_l.webp' : faces[list.length % faces.length];
      ...
      drift(dx) {
        /* 歩行中は妖精なのでふわっと追従（現状は位置固定＝一緒に移動して見える） */
      },
      burstAll() {
        ...
        snapshot.forEach((el, k) => {
          setTimeout(() => {
            ...
            img.src = ASSET + 'fat_8_haretsu.webp';
            SND.burst();
            el.animate(
            ...
            setTimeout(() => {
              img.src = ASSET + 'fat_9_shometsu.webp';
              el.style.opacity = '0';
            }, 220);
            setTimeout(() => {
              el.remove();
    ...
    return {
      show(files) {
        ...
        files.forEach((f, i) => {
          ...
          d.animate(
            [
              { transform: 'translateX(70vw)', opacity: 0.4 },
  ...
  const STORY = (() => {
    ...
    return {
      set(n) {
        segs.forEach((s, i) => s.classList.toggle('is-done', i < n));
      },
    };
  })();
  ...
  const UI = (() => {
    ...
    function say(name, text) {
      ...
      while (talk.scrollHeight > talk.clientHeight + 2 && size > 12.5) {
        // 可読性の下限12.5px
        size -= 0.5;
        textEl.style.fontSize = size + 'px';
      }
    ...
    function buttons(items) {
      // items: [{label, value}] → Promise<value>（単一選択・radio）
      return new Promise((res) => {
        chEl.innerHTML = items
          .map(
            (it) => `
    ...
    function multi(items, okLabel) {
      // 複数選択 → Promise<value[]>（checkbox）
      return new Promise((res) => {
        chEl.innerHTML =
          items
            .map(
        ...
        $(chEl)
          ...
          .on('change', function () {
            ...
            $ok.toggleClass(
              'is-hidden',
              $(chEl).find('.opt__input:checked').length === 0,
            );
          });
        $ok.one('click', () => {
          SND.tap();
          const vals = $(chEl)
            .find('.opt__input:checked')
            .map((_, el) => $(el).val())
            .get();
          chEl.innerHTML = '';
          res(vals);
      ...
      return new Promise((res) => {
        ...
        requestAnimationFrame(() => {
          if (body.scrollHeight > body.clientHeight + 4) {
            ...
            body.addEventListener('scroll', () => hint.remove(), {
              once: true,
              passive: true,
            });
          }
      ...
      return new Promise((res) => {
        ...
        const updateDots = () => {
          const i = Math.min(
            cards.length - 1,
            Math.round(
              caro.scrollLeft / (caro.firstElementChild.offsetWidth + 10),
            ),
          );
          dots.forEach((d, k) => {
            d.innerHTML = k === i ? '<b>●</b>' : '●';
    ...
    for (let k = 0; k < n; k++) {
      setTimeout(
        () => {
          ...
          setTimeout(() => {
            sy.classList.remove('is-jab');
            fl.classList.remove('is-show');
          }, 140);
          setTimeout(() => {
            story.classList.remove('is-shake');
          }, 330);
        },
        300 + k * 330,
      );
  ...
  function store_get(k) {
    try {
      return storeOk ? localStorage.getItem(k) : (storeMem[k] ?? null);
    } catch (e) {
      return storeMem[k] ?? null;
    }
  }
  function store_set(k, v) {
    try {
      if (storeOk) localStorage.setItem(k, v);
      else storeMem[k] = v;
    } catch (e) {
      storeMem[k] = v;
    }
  ...
  async function pickClinic(areaId) {
    ...
    try {
      ...
      $select
        .empty()
        .append(
          $('<option>').attr('value', '').text('現在、院一覧を取得できません'),
        )
        .prop('disabled', true);
    }
    ...
    return new Promise((resolve) => {
      if (isError) {
        $(chEl).prepend(
          '<div class="reserve__clinic-error">現在、院一覧の取得が混み合っております。院を選択せずにお進みください。ご予約後にトーク画面でご希望の院をお知らせください。</div>',
        );
      ...
      $ok.one('click', () => {
        ...
        const clinicId = isError
          ? ''
          : $select.find('option:selected').data('clinic-id');
        commitAnswer(
          'clinic_shop',
          name,
          isError ? { 'data-clinic-list-error': '' } : null,
        );
        chEl.innerHTML = '';
  ...
  function pickCalendar(clinicId) {
    const chEl = UI.el;
    return new Promise((resolve) => {
      chEl.innerHTML = `
      ...
      // この内容で仮押さえ！ボタンの入力チェック（希望日＋第1〜第3希望時間を必須にする）
      const validateCalendar = () => {
        // カレンダー表示エラー時は日時未確定のまま進む（既存仕様を維持）
        if ($('#js-time-calendar[data-tc-is-error]').length > 0) return '';
        const messages = [];
        if (!$('[name="date_1"]:checked').val())
          messages.push('希望日を選択してください');
        if (!$('[name="date_1_time_1"]').val())
          messages.push('第1希望時間を選択してください');
        if (!$('[name="date_1_time_2"]').val())
          messages.push('第2希望時間を選択してください');
        if (!$('[name="date_1_time_3"]').val())
          messages.push('第3希望時間を選択してください');
      ...
      const updateOk = () => {
        ...
        const hasDate = $('[name="date_1"]:checked').length > 0;
        $ok.toggleClass('is-hidden', !(isError || hasDate));
      };
      cal.createCalendar({ params: { clinicId, days: 21 } }).then(updateOk);
      $('#js-time-calendar').on(
        'change',
        '.cal__day-input, [name^="date_1_time_"]',
        updateOk,
      );

      $ok.on('click', () => {
        const message = validateCalendar();
        if (message) {
          alert(message);
          return;
        }
        SND.tap();
        $ok.off('click');
        // 選択済みの日付・希望時間（date_1 / date_1_time_1〜3）を保持したまま choices パネル外へ退避する
  ...
  async function story() {
    ...
    UI.say(
      '',
      'ある夜のこと。ゆいさんはベッドの中で、ぼんやり考えていました。',
    );
    await UI.tapNext();
    ...
    UI.say(
      'ナレーション',
      'その夜、ゆいさんは——これまでのダイエットの思い出を振り返りながら、ゆっくり眠りにつきました……。',
    );
    ...
    UI.say(
      'ナレーション',
      'ここは、ゆいさんの<b>夢の中</b>。今夜は<b>あなた</b>も一緒に、思い出の道を——少しだけ、歩いてみましょう。',
    );
    ...
    UI.say(
      'ナレーション',
      'ありがとうございます。年代はどちらですか？<br><small>（診断の精度が上がります）</small>',
    );
    answers.age = await UI.buttons([
      { label: '17歳以下・高校生', value: '17歳以下・高校生' },
      { label: '18〜19歳 ※高校生を除く', value: '18〜19歳' },
      { label: '20代', value: '20代' },
      { label: '30代', value: '30代' },
      { label: '40代', value: '40代' },
      { label: '50代', value: '50代' },
    ...
    UI.say(
      'ナレーション',
      '鏡を見るたび気になるのは、どのあたりですか？<br><small>（当てはまるもの全部どうぞ）</small>',
    );
    answers.body_parts = await UI.multi([
      { label: 'アゴ下・フェイスライン', value: 'アゴ下・フェイスライン' },
      { label: '二の腕', value: '二の腕' },
      { label: 'お腹（上部）', value: 'お腹（上部）' },
      { label: 'お腹（下部）', value: 'お腹（下部）' },
      { label: 'ウエスト・脇腹', value: '側腹部' },
      { label: '太もも（外側）', value: '太もも（外側）' },
      { label: '太もも（内側）', value: '太もも（内側）' },
      { label: 'お尻', value: 'お尻' },
      { label: '背中・ハミ肉', value: '背中・ハミ肉' },
    ]);
    ...
    if (answers.body_parts.length) {
      ...
      UI.say(
        'ナレーション',
        '……実は、もうお気づきかもしれません。いま選んだ部位のぶんだけ——<b>脂肪くんが、すでに一緒に歩いている</b>ことに。',
      );
      // 部位ぶんの脂肪くん（最大4匹まで＝後の失敗イベント分の枠を残す）
      answers.body_parts
        .slice(0, 4)
        .forEach((_, i) => setTimeout(() => FAT.add('S'), 700 + i * 450));
    ...
    UI.say(
      'ナレーション',
      'これまでに試したことがあるダイエット、正直に教えてください。',
    );
    answers.diet_history = await UI.multi([
      {
        label: '食事制限（サラダだけ・置き換え）',
        value: '食事制限（サラダだけ・置き換え）',
      },
      { label: '運動・ジム', value: '運動・ジム' },
      { label: 'サプリ・酵素', value: 'サプリ・酵素' },
      { label: 'エステ・マッサージ', value: 'エステ・マッサージ' },
    ...
    UI.say(
      'ナレーション',
      '……正直にありがとうございます。それでは、その頑張りの思い出を、少しだけ一緒にたどってみましょう。',
    );
    await UI.tapNext();
    ...
    const EVENTS = {
      食事制限: {
        ...
        result:
          'ゆいさんも同じでした。数日後、体はガス欠でフラフラ。「もう限界！」——反動で、食欲が爆発してしまいました。',
        pose: 'pose_4_naki.webp',
        fat: 'M',
        learn:
          '食事を極端に減らすと、体は<em>省エネモード</em>に切り替わります。消費カロリーまで減って、むしろ<b>やせにくい体</b>になってしまうんです。',
      },
      ドカ食い: {
        // 食事制限の連鎖イベント（我慢→反動を一続きの物語で見せる）
        bg: 'bg_room.webp',
        ...
        result:
          '我慢していた分だけ、反動は大きくなります。この夜、ゆいさんの脂肪くんは<b>いつもより大きく</b>育ちました。',
        pose: 'pose_2_shock.webp',
        fat: 'L',
        learn:
          'リバウンドのたびに、脂肪細胞は<em>パンパンに再肥大</em>します。我慢→反動のループが、実はいちばんの遠回りなんです。',
      },
      運動: {
        bg: 'bg_gym.webp',
        props: ['prop_5_dumbbell.webp'],
        intro:
          '入会金を払って、最初の月だけ通ったジム……心当たりはありませんか？',
        choices: [
        ...
        result:
          '続かないのは、意志が弱いからではありません。でも、やめた途端に体重はじわじわ元通り……。',
        pose: 'pose_3_onaka.webp',
        fat: 'M',
        learn:
          '運動で脂肪は<b>小さく</b>なります。でも脂肪細胞の<em>“数”はそのまま</em>。やめれば、元のサイズに戻るだけなんです。',
      },
      サプリ: {
        bg: 'bg_room.webp',
        ...
        result:
          '翌朝、体重計の数字は——昨日と同じ。「飲んだから大丈夫」の安心感だけが残りました。',
        pose: 'pose_5_kangae.webp',
        fat: 'M',
        learn:
          'サプリだけで脂肪細胞の<em>“数”が減ることはありません</em>。「飲んだから大丈夫」が、むしろ食欲の言い訳になりがちなんです。',
      },
      エステ: {
        bg: 'bg_room.webp',
        ...
        result:
          '施術後はスッキリ。でも数日たつと、シルエットはいつも通りに……。',
        pose: 'pose_5_kangae.webp',
        fat: 'M',
        learn:
          'エステで流れるのは主に<em>水分（むくみ）</em>。脂肪細胞そのものの数は変わらないため、数日で戻りやすいんです。',
      },
      甘いもの: {
        // 全員共通のフィナーレイベント（夢の散歩は必ず街に出る）
        bg: 'bg_street.webp',
        props: ['prop_2_parfait.webp', 'prop_3_cake.webp'],
        intro:
          'そして誰にでもある——がんばった日の、ご褒美スイーツ。先週は何回ありましたか？',
        choices: [
        ...
        result:
          'ご褒美は、悪いことではありません。ただ——脂肪くんにとってもご褒美だった。それだけのことでした。',
        pose: 'pose_5_kangae.webp',
        fat: 'M',
        learn:
          '糖は脂肪細胞の大好物。ご褒美のたびに、脂肪くんはこつこつ<em>“貯金”</em>を増やしていきます。',
      },
    };

    // Q4の選択肢valueとEVENTSキーを対応させるマップ（選択肢の表示文言が長いため）
    const DIET_EVENT_KEY = {
      '食事制限（サラダだけ・置き換え）': '食事制限',
      '運動・ジム': '運動',
      'サプリ・酵素': 'サプリ',
      'エステ・マッサージ': 'エステ',
    };
    ...
    const picked = answers.diet_history
      .filter((v) => v !== '特になし')
      .map((v) => DIET_EVENT_KEY[v] || v)
      .slice(0, 3);
    const queue = [];
    picked.forEach((k) => {
      queue.push(k);
      if (k === '食事制限') queue.push('ドカ食い');
    });
    queue.push('甘いもの');
    ...
    if (FAT.count() > 0) {
      UI.say(
        'ナレーション',
        `ふと振り返ると——あなたの後ろに、<b>脂肪くん×${FAT.count()}</b>。`,
      );
      await UI.tapNext();
    }
    GIRL.start();
    UI.say(
      'ナレーション',
      '……がんばってなかったわけじゃない。<b>やり方が、脂肪くんに効いていなかっただけ</b>なんです。',
    );
    ...
    UI.say(
      'ナレーション',
      'ここで、ゆいさんが長年知らなかった<b>脂肪くんのひみつ</b>を、特別にお話しします。',
    );
    await UI.tapNext('▶ ひみつを聞く');

    await POP.show({
      title: '脂肪くんのひみつ',
      html: '大人になってから、脂肪細胞の<b>“数”</b>はほとんど変わらないことが分かっています。<br>ダイエットで変わるのは、<b>“大きさ”だけ</b>。<img src="img/fat.webp" width="349" height="281" loading="lazy" alt="脂肪細胞の変化イメージ" class="pop__img">',
      btn: '▶ つまり…？',
    });
    ...
    UI.say(
      'ナレーション',
      'ダイエットをがんばると、脂肪くんはたしかに<b>小さく</b>なります。',
    );
    await UI.tapNext();
    UI.say(
      'ナレーション',
    ...
    UI.say(
      'ナレーション',
      'そして食べれば、ぷくーっと元どおり。<br>——これが、<b>リバウンドの正体</b>です。',
    );
    await UI.tapNext();
    GIRL.pose('pose_6_hirameki.webp');
    UI.say(
      'ゆい',
      '「じゃあ……脂肪くんの<b>“数”そのもの</b>を減らせたら、もう戻らないんじゃない？」',
    );
    ...
    UI.say(
      'ナレーション',
      'あります。脂肪細胞を、<b>壊して、外に出す。</b><br>JUNOの<b>スルリム式・脂肪破壊術</b>です。',
    );
    await UI.tapNext('▶ え？スルリムって何？');

    UI.say(
      'ナレーション',
      '<b>韓国で大バズした施術</b>を、JUNOが日本人の体質に合わせて<em>独自にアップデート</em>したもの——それがスルリム式です。',
    );
    await UI.tapNext('▶ 安全性は？');

    await POP.show({
      title: 'はじめに、安心してください',
      html: 'スルリム式 脂肪破壊術は<br><b class="pop__accent">10年無事故<small>※</small></b> で安全性の高い施術です。<div class="badges"><div>韓国で<br>独自開発</div><div>治験検証<br>済み</div><div>症例実績<br>15万人</div></div><small class="pop__note">※期間: 2015/6/30-2025/6/30<br>※施術直後は一時的に腫れ感が出る場合があります</small>',
      btn: '▶ くわしく知る',
    });
    ...
    const REASONS = [
      [
        '1回で従来の約5回分',
        'FDA承認のデオキシコール酸を高濃度配合。少ない回数で効果を実感いただけます。',
      ],
      [
        '脂肪細胞を最大35%破壊',
        '脂肪細胞の“数”自体を減らすため、リバウンドしにくい仕組みです。',
      ],
      [
        '1部位およそ15分',
        '切開不要・麻酔不要。お仕事帰りにも通えます。当日シャワーOK。',
      ],
      [
        'ダウンタイムほぼなし',
        '翌日からメイク可能。一時的な腫れや内出血は通常1〜2週間で改善します。',
      ],
      [
        '韓国で大バズの超人気メニュー',
        '韓国で爆発的に流行した施術を、JUNOが日本人の体質に合わせて独自にパワーアップ。',
      ],
    ];
    await POP.carousel({
      ...
      cards: REASONS.map(
        (r, i) =>
          `<b class="caro__reason-num">${i + 1}. ${r[0]}</b><img src="img/sl_illust_0${i + 1}.webp" width="647" height="258" loading="lazy" alt="${r[0]}" class="pop__img">${r[1]}`,
      ),
      btn: '▶ なるほど…！',
    });
    ...
    if (fatN > 0) {
      UI.say(
        'ナレーション',
        'それでは——ゆいさんの後ろの脂肪くんたちで、<b>実際にお見せしましょう</b>。',
      );
    ...
    UI.say(
      'ナレーション',
      '小さくするのではなく、<b>数ごと、いなくなる</b>。<br>だから「<b>リバウンド知らずの脂肪破壊術</b>」なんです。',
    );
    await UI.tapNext();
    const BA_CASES = [
    ...
    await POP.carousel({
      ...
      cards: BA_CASES.map(
        (c, i) =>
          `<div class="ba-case__head"><span class="ba-case__badge"></span><b class="ba-case__title">${c[0]}</b></div>
        <img src="img/ba-0${i + 1}.webp" width="526" height="400" loading="lazy" alt="${c[0]} 症例写真（Before/After）" class="ba-case__img">
        <div class="ba-case__note">♡ ${c[1]}</div>
        <small class="ba-case__disclaimer">※個人の感想です。効果には個人差があります</small>`,
      ),
      btn: '▶ あなたの場合は？',
    });
    ...
    // ===== S6 特典（4枚ポンポン出現・タップは1回だけ・診断書と同じ進捗セグメント） =====
    ...
    UI.say(
      'ナレーション',
      'このGIFTは、<em>24時間以内にLINEのトーク画面で受け取る</em>と有効になります（有効期限は画面上部に表示しています）。',
    );
    await UI.tapNext();
    UI.say(
      'ナレーション',
      '現在スルリム式は大変混み合っていて、<em>人気の時間帯は1〜2週間待ち</em>になることも。ですが、GIFT 04の権利で——<em>このページ限定の優先仮押さえ</em>ができます。<br><small>（無料・キャンセル自由です）</small>',
    );
    ...
    UI.say(
      'ナレーション',
      '施術は<b>1部位およそ15分</b>で完了。お仕事帰りや<b>スキマ時間の活用</b>も人気です。——お住まいのエリアはどちらですか？<br><small>（仮押さえ 1／3）</small>',
    );
    const areaId = await pickArea();
    UI.say(
      'ナレーション',
      `<b>${AREA_LABELS[areaId] || ''}</b>のお近くの院をお選びください。<br><small>（仮押さえ 2／3）</small>`,
    );
    const clinicResult = await pickClinic(areaId);
    answers.clinic = clinicResult.name;
    UI.say(
      'ナレーション',
      'あと少し！ご希望の日時を<b>第1〜第3希望</b>までタップしてください。<br><small>（仮押さえ 3／3）</small>',
    );
    await pickCalendar(clinicResult.clinicId);
    UI.say(
      'ナレーション',
      `${answers.clinic ? `<b>${answers.clinic}</b> でのご希望、` : 'ご希望、'}お預かりしました。<em>LINE追加で仮押さえが確定</em>します。<br><small>※無料・キャンセル自由。確定のご連絡はLINEでお届けします</small>`,
    );
    await UI.tapNext();
    ...
    await POP.show({
      title: 'あなたが手に入れたもの',
      html: `✔ 脂肪タイプ診断書（${ft.name}）<br>✔ GIFT 01: 1部位目 4,900円<br>✔ GIFT 02: 2部位目以降 14,800円<br>✔ GIFT 03: 人気施術1つ無料<br>${answers.clinic ? `✔ ${answers.clinic} の仮押さえ` : '✔ GIFT 04: 優先仮押さえ権（LINEから使えます）'}<br><br><b class="pop__strong">LINE追加で、すべて受け取れます。</b><br><small>診断書の保存・特典チケットの発行・仮押さえの確定連絡が届きます</small>`,
      btn: '▶ 受け取りに進む',
    });
    // クロージング（社会的証明 → 損失回避 → 安心 → CTA）
    UI.say(
      'ナレーション',
      'この診断を受けた方の<em>9割以上</em>が、あなたと同じ悩みを抱えていました。そして——<b>変わりはじめた人から、夏を楽しんでいます</b>。',
    );
    await UI.tapNext();
    UI.say(
      'ナレーション',
      '特典チケットは<em>24時間で消え、1部位目は元の金額に戻ります</em>。受け取りは<b>LINE追加だけ・約10秒</b>。カウンセリングは無料で、<b>無理な勧誘は一切ありません</b>。',
    );
    await UI.tapNext();
    UI.say(
      'ナレーション',
      'スルリムは、<b>痩せたいけど食べたい</b>——運動や食事制限なしで変わりたい、そんな方のための施術です。脂肪細胞の“数”から変えて、<em>食べても戻りにくい体へ</em>。',
    );
    await UI.tapNext();
    UI.say(
      'ナレーション',
      'この夏は思う存分、<em>脂肪を破壊して体の外へ</em>出しましょう。',
    );
    await UI.tapNext();
    confetti(30);
    UI.say(
      'ゆい',
      '「がんばり方を、変えるだけ。<em>今年の夏は、我慢しないで楽しみましょ</em>」',
    );

    const chFinal = UI.el;
    chFinal.innerHTML = `
      <div class="closing__badges">
        ${['カウンセリング無料', '無理な勧誘なし', '通知オフ・ブロック自由']
          .map((t) => `<span class="closing__badge">✓ ${t}</span>`)
          .join('')}
      </div>
  ...
  // すべての const 定義（postAdCountStatus 等）が初期化された後に実行する
  init();
});
